    ...
    public class Verb_ShootWeaponAbility : Verb_Shoot
    {
        protected override bool TryCastShot()
        {
            ...
            if (this.verbProps.ForcedMissRadius > 0.5f)
            {
                ...
                if (num > 0.5f)
                {
                    ...
                    if (num2 > 0)
                    {
                        ...
                        projectile2.Launch(launcher, drawPos, c, this.currentTarget, projectileHitFlags, true, equipment);
                        return true;
                    }
                }
            }
            ...
            if (!Rand.Chance(shotReport.AimOnTargetChance_IgnoringPosture) && !cannotMiss)
            {
                ...
                projectile2.Launch(launcher, drawPos, shootLine.Dest, this.currentTarget, projectileHitFlags2, true, equipment);
                return true;
            }
            if (this.currentTarget.Thing != null && this.currentTarget.Thing.def.category == ThingCategory.Pawn && !Rand.Chance(shotReport.PassCoverChance) && !cannotMiss)
            {
                ...
                if (!TryConsumeAmmunition()) return false;
                projectile2.Launch(launcher, drawPos, randomCoverToMissInto, this.currentTarget, projectileHitFlags3, true, equipment);
                return true;
            }
            ...
            {
                if (!TryConsumeAmmunition()) return false;
                projectile2.Launch(launcher, drawPos, this.currentTarget, this.currentTarget, projectileHitFlags4, true, equipment);
                this.ThrowDebugText("Hit\nDest", this.currentTarget.Cell);
            }
            else
            {
                if (!TryConsumeAmmunition()) return false;
                projectile2.Launch(launcher, drawPos, shootLine.Dest, this.currentTarget, projectileHitFlags4, true, equipment);
                this.ThrowDebugText("Hit\nDest", shootLine.Dest);
            }
