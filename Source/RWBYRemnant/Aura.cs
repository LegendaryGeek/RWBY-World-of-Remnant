    ...
    public class Aura : IExposable
    {
        ...
        public virtual bool TryAbsorbDamage(DamageInfo dinfo)
        {
            ...
            else // break
            {
                ...
                FleckMaker.Static(loc, pawn.Map, FleckDefOf.ExplosionFlash, 12f);
                for (int i = 0; i < 6; i++)
                {
                    Vector3 loc1 = pawn.TrueCenter() + Vector3Utility.HorizontalVectorFromAngle((float)Rand.Range(0, 360)) * Rand.Range(0.3f, 0.6f);
                     FleckMaker.ThrowDustPuff(loc1, pawn.Map, Rand.Range(0.8f, 1.2f));
                }
                currentEnergy = 0f;
            }
